...
export default async function MediaList({ searchParams, lang }) {
  ...
  const getPageNumbers = () => {
    ...
    if (totalPages <= pageLimit) {
      ...
      if (end === totalPages) start = Math.max(1, totalPages - pageLimit + 1);
    ...
    <div className="py-10 md:py-20 px-6">
      <div className="max-w-5xl mx-auto ">
        ...
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b-[3px] border-gray-950">
          ...
          <form className="relative w-full md:w-72" role="search">
        ...
        <div className="flex flex-col gap-3" role="list">
          {news?.length > 0 ? (
            news.map((item) => {
              const newsTitle = isEnglish
                ? item.news_title_en || item.news_title
                : item.news_title;
              const pressName = isEnglish
              ...
              return (
                <article key={item.id} role="listitem">
                  <a
                    ...
                    title={`${newsTitle} - ${pressName} ${t("기사 읽기", "Read article")}`}
                  >
                    <div className="flex items-center md:flex-col md:items-start gap-3 md:gap-1 shrink-0 w-28">
                      <time
                        ...
                        className="text-[11px] font-black text-gray-300 font-mono tracking-tighter"
                      >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-bold text-gray-950 group-hover:text-blue-600 leading-snug break-keep transition-colors">
                        {newsTitle}
                      </h3>
                    </div>
