...
const PostList = async ({
  ...
  return (
    <div className="max-w-5xl mx-auto relative z-10">
      <section className="mb-16 pb-10 border-b-2 border-gray-950 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <h2 className="text-4xl md:text-5xl font-black tracking-tighter text-gray-950">
            {title}
          </h2>
        </div>

        {/* role="search" 추가하여 검색 영역임을 명시 */}
        <form className="flex justify-end w-full md:w-auto" role="search">
      ...
      {/* LIST: 목록임을 명시하는 role="list" 및 각 항목을 article로 구성 */}
                ...
                >
                  <div className="shrink-0 flex flex-col items-start md:items-center w-20">
                    ...
                    <time
                      ...
                      className="text-[13px] font-black text-gray-400 group-hover:text-gray-900 transition-colors"
                    >
                  ...
                  <div className="flex-1 min-w-0">
                    {/* 게시물 제목은 h3로 위계 설정 */}
                    <h3 className="text-xl font-black text-gray-950 tracking-tighter group-hover:text-blue-600 transition-colors truncate">
                      {postTitle}
                    </h3>
                  </div>
                  <div
                    className="hidden md:block opacity-0 group-hover:opacity-100 transition-all translate-x-[-10px] group-hover:translate-x-0"
                    aria-hidden="true"
                  >
                    <svg
                      width="24"
      ...
      {/* PAGINATION: nav 태그와 aria-label로 탐색 영역임을 명시 */}
      {totalPages > 1 && (
        <nav
